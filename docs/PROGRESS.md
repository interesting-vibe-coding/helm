...
**2026-06-14 rename 轮开题 + Stage A(品牌可见字串)+ Help-menu GitHub 改向**:
- **审计**: helm/Kaku→Kaji 是**三层纠缠** — identity 已 Kaji(bundle id `dev.kaji.app` / CFBundleName / release.yml `Kaji.app.zip` / kaku_theme Kaji alias),但 guts 仍 helm/Kaku:binary `helm`/`helm-gui`、bundle 目录 `Helm.app`、URL scheme `helm://`、`~/.config/helm`+`~/.helm`+data dir、Lua `Helm` 表(136 用)、10 个 `tools/helm-*`、菜单栏 "Kaku"、脚本 `APP_NAME="Kaku"`。三 Explore agent 全量勘察(file:line + 风险轴 codesign/TCC/用户数据迁移/行为耦合)。方案存 `~/.claude/plans/recursive-wobbling-cook.md`,分 A/B/C 三阶。
- **Stage A(PR #212 MERGED)— 安全, 纯 in-app 显示字串**: 用户最常见的 chrome → "Kaji"。`commands.rs`: app-menu group key "Kaku"→"Kaji"(order vec + rank match + assign_as_app_menu 判断, 五处 lockstep)、About `Kaku V{ver}`→`Kaji V{ver}`、Hide/Quit、Kaji Doctor、Apply Last AI Fix doc。`termwindow/mod.rs`: AI toast 显示文案 "Kaku Assistant"→"Kaji Assistant"(toast ID `kaku-toast-*` 保留=行为键)、bell 通知 title "Kaku"→"Kaji"、analyzing/generating/default-terminal toast。**故意不碰**(行为/identity, 留后阶): TERM_PROGRAM、DA2 term_program、scheme 注册名 'Kaku Dark'/'Kaku Light'、binary/Helm.app 发现路径、URL scheme、doctor.rs 散文(与 Helm.app 路径+kaku wrapper 纠缠)。
- **Help-menu GitHub 改向(PR #213)**: `commands.rs` 里 Help 菜单(Star / Report Issue)+ rank/CommandDef 的 `github.com/tw93/Kaku`(上游 fork 源)全部 10 处 → `interesting-vibe-coding/kaji`。**launch 关键**: 别把 star/issue 送去上游仓。kaji repo 实查: issues ✓ 开 / discussions ✗ 关(discussions 那条 CommandDef `menubar:&[]`=未挂出, vestigial, 安全)。`crates/*/Cargo.toml` 的 `repository=tw93/Kaku` 不动(vendored 上游元数据, 非用户面)。
- **发现两个独立 bug(留后修, 非本轮)**: ① `scripts/check_config_release_readiness.sh` **双层坏** — 路径钉死不存在的 `Kaku.app`(改 Helm.app 后揭出第二层)+ 断言 bundled kaku.lua 应 `读 config_version.txt` 但它根本没引用(config_version:22 机制需查)。本轮把试探性路径改 revert 了, 保 Stage A 纯净。② doctor.rs ~35 处 "Helm" 散文 + tools/helm-* → 跟 binary/bundle/path 一起在 Stage C 整体改(现改成 Kaji 会半吊子)。
- **下一步**: Stage B(crate 名 kaku→kaji, rebuild-only) / **Stage C(identity 改名: binary+bundle 目录+URL scheme+配置/会话目录迁移 shim, 需真签名发版 + 你重授 TCC + 迁移测试, 会动你在用的 ~/.config/helm)** — C 要你在场再做。
