#!/usr/bin/env bash
# dev-link.sh — wire an installed Kaji.app + your user config to THIS repo so you
# see changes instantly, without rebuilding:
#   • kaku.lua          → edit repo, press Cmd+R in Kaji to hot-reload (0 build)
#   • tools/ first_run/ skills/ → edit repo, takes effect immediately (next run)
#   • Rust code         → still needs `./scripts/build.sh` (low frequency)
#
# Run once after installing Kaji. Re-run safely (idempotent). Use ./scripts/undev-link.sh to revert.
set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BREPO="$REPO/assets/macos/Kaji.app/Contents/Resources"
APP="/Applications/Kaji.app/Contents/Resources"

green() { printf '\033[0;32m%s\033[0m\n' "$*"; }

# 1) kaku.lua: user config → repo (the file Kaji actually loads). Cmd+R reloads it.
#    Kaji now reads ~/.config/helm; keep the legacy ~/.config/kaku symlink too so a
...
if [[ -d "$APP" ]]; then
  ...
  green "✓ Kaji.app Resources (tools/ first_run.sh/ skills/) → repo  (instant)"
else
  printf '  (Kaji.app not installed; skipped bundle links)\n'
fi

echo
green "Dev mode on. Edit files in $REPO directly."
echo "  • kaku.lua  → ./scripts/reload.sh  (or Cmd+R in Kaji)"
